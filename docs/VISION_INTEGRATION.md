...
- **Schema** (`kirana/repositories/base.py::_ensure_schema` + `db_generation/ensure_full_schema.py`, boot-migrated): `vision_session`, `vision_item` (+`bbox_json`, `image_index`), `counter_session`, `counter_item`.
...
- **Endpoints** (`/kirana/vision`): `shelf/analyze`, `sessions`, `session/{id}/items`, `sales`, `correct/{id}`, **`item/{id}/crop`**, `image/{path}`, `onboarding/{analyze,commit/{id}}`, `counter/{sync,summary}`.
