    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.menu_book_outlined), selectedIcon: Icon(Icons.menu_book_rounded), label: 'Khata'),
          NavigationDestination(icon: Icon(Icons.storefront_outlined), selectedIcon: Icon(Icons.storefront_rounded), label: 'Billing'),
        ],
      ),
    );
  }
...
class _RequestTrialScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_RequestTrialScreen> createState() => _RequestTrialScreenState();
}
...
class _RequestTrialScreenState extends ConsumerState<_RequestTrialScreen> {
  ...
  Future<void> _request() async {
    ...
    try {
      await ref.read(subscriptionProvider.notifier).requestTrial(tier: _selectedTier);
    } catch (_) {
    } finally {
            ...
            children: [
              Center(
                child: Container(
                  ...
                  child: const Icon(Icons.store_mall_directory_rounded, size: 40, color: BrandColors.primary),
                ).animate().scale(duration: 500.ms, curve: Curves.elasticOut),
              ),
              const SizedBox(height: 24),
              const Text(
                'Welcome to Kirana AI',
                style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: BrandColors.ink),
                textAlign: TextAlign.center,
              ).animate().fadeIn(delay: 150.ms),
              const SizedBox(height: 8),
              const Text(
                'Choose a plan to trial free. Our team will activate it shortly.',
                style: TextStyle(fontSize: 14, color: BrandColors.muted, height: 1.5),
                textAlign: TextAlign.center,
              ).animate().fadeIn(delay: 200.ms),
              const SizedBox(height: 28),

              const Text('SELECT YOUR TRIAL PLAN',
                  style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, letterSpacing: 1.4, color: BrandColors.muted)),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text('Request ${_selectedTier == 'pro' ? 'Pro' : 'Basic'} Trial'),
                  style: ElevatedButton.styleFrom(
                    minimumSize: const Size.fromHeight(56),
                    backgroundColor: _selectedTier == 'pro' ? const Color(0xFF7C3AED) : BrandColors.primary,
              ...
              Center(
                child: TextButton(
                  ...
                  child: const Text('Sign in to a different account', style: TextStyle(color: BrandColors.muted)),
                ),
              ),
            ],
          ),
        ...
        child: Row(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(color: selected ? color : BrandColors.border, width: 2),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: selected ? color : BrandColors.ink),
                      ),
                      if (badge != null) ...[
                        const SizedBox(width: 6),
                        Container(
                  ...
                  ...features.map((f) => Padding(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 6),
                      Expanded(child: Text(f, style: const TextStyle(fontSize: 12, color: BrandColors.ink))),
                    ]),
                  )),
                ],
              ),
