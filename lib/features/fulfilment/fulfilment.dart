...
class _FulfilmentScreenState extends ConsumerState<FulfilmentScreen>
                    ...
                    child: Row(children: [
                      Expanded(
                        child: Column(
                            ...
                            children: [
                              Text(e['customer_name']?.toString() ?? 'Customer',
                                  style: const TextStyle(
                                      fontWeight: FontWeight.w700)),
                              Text('#${e['estimate_id']} · ${e['status']}',
                                  style: const TextStyle(
                                      fontSize: 12, color: BrandColors.muted)),
                            ]),
                      ),
                      Text('₹${(e['total'] as num?)?.toStringAsFixed(0) ?? "0"}',
                ...
                itemBuilder: (_, i) {
                  ...
                  return Container(
                    ...
                    child: Row(children: [
                      ...
                      Text('₹${(r['refund_amount'] as num?)?.toStringAsFixed(0) ?? "0"}',
                          style:
  ...
  void _addReturn() {
    ...
    bool exchange = false;
    _sheet('Log return / exchange', [
      TextField(controller: orderId, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Order # (optional)')),
      const SizedBox(height: 12),
      TextField(controller: reason, decoration: const InputDecoration(labelText: 'Reason')),
      const SizedBox(height: 12),
      TextField(controller: refund, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Refund ₹')),
      StatefulBuilder(
      ...
      await ref.read(apiClientProvider).post('/kirana/sales-returns', {
        if (orderId.text.trim().isNotEmpty) 'order_id': int.tryParse(orderId.text.trim()),
